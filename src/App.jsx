...
const generateExcel = (contract, items, settlement) => {
  ...
  const info=[["卖方（供应商）：",contract.seller,"合同编号：",contract.contract_no],["买方（客户）：",contract.buyer,"对账日期：",formatLocalDate()],["联系人（卖方）：",contract.seller_contact||"","合同日期：",contract.contract_date],["联系人（买方）：",contract.buyer_contact||"","送货单号：",contract.delivery_no||""]];
  info.forEach(([l1,v1,l2,v2],i)=>{ const r=2+i; merge(r,r,1,1);set(r,1,l1,ST.label); merge(r,r,2,6);set(r,2,v1,ST.value); merge(r,r,7,7);set(r,7,l2,ST.label); merge(r,r,8,10);set(r,8,v2,ST.value); });
  ["序号","物料名称","规格/描述","颜色","单位","合同数量\n(套)","实送数量\n(件)","含税单价\n(元/套，13%)","含税金额\n(元)","备注"].forEach((h,ci)=>set(6,ci+1,h,ST.header));
  items.forEach((item,i)=>{ const r=7+i; const isA=i%2===1; const cs=isA?ST.cellA:ST.cell; const csl=isA?ST.cellLA:ST.cellL; [i+1,item.name,item.spec,item.color||"白色",item.unit||"件",item.contract_qty,toNumber(item.delivered_qty,0),"","",item.note||""].forEach((v,ci)=>set(r,ci+1,v,ci===2?csl:cs)); });
  ...
  [`1. 本对账单依据采购合同（${contract.contract_no}）及送货工单（${contract.delivery_no||"—"}）编制，对账日期：${formatLocalDate()}。`,`2. 合同约定：${contract.product_name||""} ${contract.contract_qty} 套，含税单价 ¥${contract.unit_price}/套（含增值税13%），合计 ¥${(contract.contract_qty*contract.unit_price).toFixed(2)}。`,`3. 本次结算数量：${settleQty} 套，结算金额：¥${totalAmt.toFixed(2)}（${contract.settlement_mode==="actual"?"按实际最小送货量结算":"按合同约定数量结算"}）。`,"4. 如双方对上述金额无异议，请买方于收到本对账单后5个工作日内书面确认，逾期视为认可。"].forEach((txt,i)=>{ merge(remR+1+i,remR+1+i,1,10); set(remR+1+i,1,txt,ST.remark); });
...
const UploadBox = ({ label, onFile, file }) => {
  ...
  return (
    <div onClick={() => ref.current.click()} onDrop={onDrop} onDragOver={e=>e.preventDefault()}
      ...
      {file?(<><span style={{fontSize:28}}>✅</span><span style={{fontSize:13,color:"#16a34a",fontWeight:600}}>{file.name}</span><span style={{fontSize:11,color:"#86efac"}}>点击重新上传</span></>)
  ...
  <div style={{overflowX:"auto"}}>
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
      ...
      <tbody>{items.map((item,i)=>{
        ...
        return (<tr key={i} style={{background:i%2?"#f8fafc":"#fff"}}>
          ...
          <td style={{padding:"8px 12px",borderBottom:"1px solid #e2e8f0",textAlign:"center"}}>
            <input type="number" min="0" value={item.delivered_qty_input??""} onChange={e=>onChangeInput(i,e.target.value)} onBlur={()=>onBlurQty(i)}
              style={{width:80,textAlign:"center",padding:"4px 6px",borderRadius:6,fontSize:13,outline:"none",border:qty===null?"1px solid #cbd5e1":matched?"1px solid #22c55e":"1px solid #f97316",background:qty===null?"#fff":matched?"#f0fdf4":"#fff7ed"}}/>
          </td>
          <td style={{padding:"8px 12px",borderBottom:"1px solid #e2e8f0",textAlign:"center",color:diff===null?"#94a3b8":diff>0?"#16a34a":diff<0?"#dc2626":"#64748b",fontWeight:600}}>{diff===null?"—":diff>0?`+${diff}`:diff}</td>
          <td style={{padding:"8px 12px",borderBottom:"1px solid #e2e8f0",textAlign:"center"}}>
            {qty===null?<span style={{color:"#94a3b8",fontSize:12}}>待录入</span>
              :matched?<span style={{background:"#dcfce7",color:"#16a34a",padding:"2px 10px",borderRadius:20,fontSize:12,fontWeight:600}}>✓ 达标</span>
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="搜索合同号或客户名…"
      ...
      : list.length===0 ? <div style={{textAlign:"center",padding:40,color:"#94a3b8"}}>暂无记录</div>
      : list.map(r=>(
                ...
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                  ...
                  <tbody>{r.items.map((it,i)=>(
                    <tr key={i} style={{background:i%2?"#f1f5f9":"#fff"}}>
                      ...
                      <td style={{padding:"6px 10px",textAlign:"center",color:toNumber(it.delivered_qty,0)>=it.contract_qty?"#16a34a":"#dc2626",fontWeight:600}}>{toNumber(it.delivered_qty,0)}</td>
                    </tr>
                  ))}</tbody>
                </table>
              )}
...
export default function App() {
  ...
  const updateQtyInput = (i,raw) => setItems(prev=>prev.map((it,idx)=>idx===i?{...it,delivered_qty_input:raw}:it));
  const commitQtyInput = (i) => setItems(prev=>prev.map((it,idx)=>{
    if(idx!==i) return it;
  ...
  const parseContract = async () => {
    ...
    try {
      ...
      const content=isPdf?[{type:"document",source:{type:"base64",media_type:"application/pdf",data:b64}}]:[{type:"image",source:{type:"base64",media_type:mime,data:b64}}];
  ...
  const parseDelivery = async () => {
    ...
    try {
      ...
      setItems(prev=>prev.map(it=>{
        ...
        return {...it,delivered_qty:match?.delivered_qty??it.delivered_qty,delivered_qty_input:match?.delivered_qty!=null?String(match.delivered_qty):(it.delivered_qty_input??""),note:match?.note||it.note};
      }));
  ...
  const anyShort = items.some(it=>{ const q=it.delivered_qty; return q===null||toNumber(q,0)<it.contract_qty; });
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#f0f4f8,#e8edf2)",fontFamily:"'PingFang SC','Hiragino Sans GB','Microsoft YaHei',sans-serif",padding:"32px 16px"}}>
      <div style={{maxWidth:880,margin:"0 auto"}}>
        ...
        <div style={{textAlign:"center",marginBottom:36}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:12,background:"#1e293b",color:"#fff",padding:"10px 24px",borderRadius:40,marginBottom:14}}>
              ...
              {step===0&&(
                <div>
                  ...
                  <button onClick={parseContract} disabled={!contractFile||loading}
                    style={{marginTop:20,width:"100%",padding:"14px 0",borderRadius:10,fontWeight:700,fontSize:15,border:"none",letterSpacing:1,cursor:contractFile&&!loading?"pointer":"not-allowed",background:contractFile&&!loading?"#1e293b":"#e2e8f0",color:contractFile&&!loading?"#fff":"#94a3b8"}}>
                ...
                <div>
                  ...
                  <div style={{background:"#f8fafc",borderRadius:10,padding:16,marginBottom:24,border:"1px solid #e2e8f0"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                      ...
                      <button onClick={resetAll} style={{fontSize:12,color:"#94a3b8",background:"none",border:"none",cursor:"pointer"}}>重新上传</button>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"5px 20px",fontSize:13}}>
                      {[["合同编号",contract.contract_no],["合同日期",contract.contract_date],["卖方",contract.seller],["买方",contract.buyer],["产品",contract.product_name],["合同数量",`${contract.contract_qty} 套`],["含税单价",`¥${contract.unit_price}`],["贸易方式",contract.trade_mode]].map(([k,v])=>(
                        <div key={k} style={{display:"flex",gap:6}}><span style={{color:"#94a3b8",minWidth:60}}>{k}：</span><span style={{color:"#1e293b",fontWeight:500}}>{v}</span></div>
                      ))}
                    </div>
                  </div>
                  ...
                  {step===1&&(
                    <>
                      ...
                      <button onClick={parseDelivery} disabled={!deliveryFile||loading}
                        style={{marginTop:20,width:"100%",padding:"14px 0",borderRadius:10,fontWeight:700,fontSize:15,border:"none",letterSpacing:1,cursor:deliveryFile&&!loading?"pointer":"not-allowed",background:deliveryFile&&!loading?"#1e293b":"#e2e8f0",color:deliveryFile&&!loading?"#fff":"#94a3b8"}}>
                  ...
                  {step===2&&(
                    <>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
                        ...
                        <button onClick={()=>{setStep(1);setDeliveryFile(null);setError("");}} style={{fontSize:12,color:"#64748b",background:"#f1f5f9",border:"none",borderRadius:6,padding:"4px 12px",cursor:"pointer"}}>重新上传送货单</button>
                      </div>
                      <div style={{padding:"10px 16px",borderRadius:8,marginBottom:16,fontSize:13,fontWeight:600,background:allMatched?"#dcfce7":"#fff7ed",color:allMatched?"#15803d":"#c2410c",border:`1px solid ${allMatched?"#86efac":"#fdba74"}`}}>
                        {allMatched?"✅ 所有物料数量已达标，可生成对账单":`⚠️ 还有 ${items.filter(it=>toNumber(it.delivered_qty,0)<it.contract_qty).length} 项数量不足，可手动修改后继续`}
                      </div>
                      ...
                      <div style={{marginTop:18,padding:"14px 16px",background:"#f8fafc",borderRadius:10,border:"1px solid #e2e8f0"}}>
                        ...
                        <div style={{display:"flex",gap:12}}>
                          {[["contract","按合同数量结算",`¥${(contract.contract_qty*contract.unit_price).toFixed(2)}`],
                            ...
                            <div key={mode} onClick={()=>setSettlementMode(mode)} style={{flex:1,padding:"10px 14px",borderRadius:8,cursor:"pointer",transition:"all .2s",border:settlementMode===mode?"2px solid #1e293b":"2px solid #e2e8f0",background:settlementMode===mode?"#f0f4f8":"#fff"}}>
                      ...
                      <button onClick={doGenerate} style={{marginTop:20,width:"100%",padding:"16px 0",borderRadius:10,fontWeight:700,fontSize:16,border:"none",cursor:"pointer",letterSpacing:2,color:"#fff",background:"linear-gradient(135deg,#16a34a,#15803d)",boxShadow:"0 4px 12px rgba(22,163,74,.3)"}}>
                        📥 生成并下载对账单 Excel
                      </button>
                      {anyShort&&<p style={{textAlign:"center",fontSize:12,color:"#f97316",marginTop:8,margin:"8px 0 0"}}>部分数量不足，仍可导出 — 将以当前填写数量生成</p>}
                      <button onClick={resetAll} style={{marginTop:12,width:"100%",padding:"10px 0",borderRadius:10,fontWeight:600,fontSize:14,border:"1px solid #e2e8f0",cursor:"pointer",color:"#64748b",background:"#f8fafc"}}>
                        🔄 新建下一张对账单
                      </button>
                    </>
                  )}
        ...
        <p style={{textAlign:"center",fontSize:11,color:"#cbd5e1",marginTop:20}}>深圳市源丰隆实业有限公司 · 对账单智能生成工具 v3 · 格式基于 P026010606 标准模板</p>
      </div>
    </div>
  );
}